...
#define UNROLL8 8
  ...
  for (u64 i = 0; i < n; i++)
  {
    for (u64 k = 0; k < n; k++)
    {
      ...
      // Unroll 4 times
      for (u64 j = 0; j < (n - (n & (UNROLL8 - 1))); j += UNROLL4)
      {
        c[i * n + j] += _a_ * b[k * n + j];
        c[i * n + j + 1] += _a_ * b[k * n + j + 1];
        c[i * n + j + 2] += _a_ * b[k * n + j + 2];
        c[i * n + j + 3] += _a_ * b[k * n + j + 3];
        c[i * n + j + 1] += _a_ * b[k * n + j + 4];
        c[i * n + j + 2] += _a_ * b[k * n + j + 5];
        c[i * n + j + 3] += _a_ * b[k * n + j + 6];
        c[i * n + j + 1] += _a_ * b[k * n + j + 7];
      }

      // Manage the leftovers
      for (u64 j = (n - (n & 8)); j < n; j++)
        c[i * n + j] += _a_ * b[k * n + j];
    }
  }
}
